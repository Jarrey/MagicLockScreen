...
{
    public class FlickrImageSearchBackgroundTaskService : BackgroundTaskService
    {
        public FlickrImageSearchBackgroundTaskService(Service service, XmlElement configXml)
        ...
        private async Task Task_Run(Dictionary<string, string> parameters)
        {
            if (parameters.ContainsKey("LockScreen") && parameters.ContainsKey("Wallpaper"))
            {
                ...
                var flickrImageSearchService = Service as FlickrImageSearchService;
                if (flickrImageSearchService != null)
                {
                    FlickrSearchImage image = await flickrImageSearchService.QueryDataAsync(parameters["keyword"], random.Next(0, (int)flickrImageSearchService.MaxItemCount));
                    if (image != null)
                    {
                        ...
                        ApplicationHelper.UpdateTileNotification(image.OriginalImageUrl,
                                                                 flickrImageSearchService.ServiceChannel.Model.Title,
                                                                 image.Title);
                    }
                }
            }
