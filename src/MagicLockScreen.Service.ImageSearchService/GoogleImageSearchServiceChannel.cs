...
{
    public class GoogleImageSearchServiceChannel : ServiceChannel, ISearchServiceChannel
    {
        private string settingsFileName = "GoogleImageSearchService.setting";
        ...
        /// <summary>
        ///     ID is [Guid("2924EE5A-E0D1-4940-9498-11A199747093")]
        /// </summary>
        public GoogleImageSearchServiceChannel(XmlElement configXml) : base(configXml)
        {
        }
        ...
        public async void InitializeServiceChannelModels()
        {
            ...
            try
            {
                ...
                if (await ApplicationData.Current.RoamingFolder.CheckFileExisted(settingsFileName))
                {
                    ...
                    {
                        AddModel(this, element.GetAttribute("Keyword"), ResourcesLoader.Loader["GoogleSearchSubTitle"]);
                    }
                    xmlSetting = null;
                }
            }
        ...
        public void AddModel(ServiceChannel serviceChannel, string queryText, string subTitle)
        {
            var model =
                new ImageSearchServiceChannelModel(serviceChannel, queryText, subTitle)
                    {
                        SearchProviderType = SearchProvider.Google
                    };
        ...
        protected override void InitializeServiceChannel(XmlElement configXml)
        {
            ...
            if (ID.CompareTo(new Guid("2924EE5A-E0D1-4940-9498-11A199747093")) != 0)
        ...
        public async void InitializeModelLogo(ImageSearchServiceChannelModel model)
        {
            try
            {
                ...
                IList<SearchImage> googleImages =
                    await googleImageSearchService.QueryDataAsync(model.Title, 0U, LogoCount);
                if (googleImages != null && googleImages.Count > 0)
                {
                    model.Logo = new Collection<BindableImage>(googleImages as IList<BindableImage>);
                    ApplicationHelper.UpdateTileNotification(googleImages[0].ThumbnailImageUrl, model.Title,
                                                             googleImages[0].Title);
                }
            }
            catch (Exception ex)
            {
