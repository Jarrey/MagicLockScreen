...
namespace MagicLockScreen_Service_LocalService.UI.Views
{
    ...
    /// </summary>
    public sealed partial class LocalPictureLibraryServiceChannelPage : LayoutAwarePage
    {
        public LocalPictureLibraryServiceChannelPage()
        {
            InitializeComponent();
            RefreshContent();
        }
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            {
                object parameters = CoreApplication.Properties[navigationParameter.ToString()];
                if (parameters is object[])
                {
                    foreach (object parameter in parameters as object[])
                    {
                        if (parameter is LocalPictureLibraryServiceChannelModel)
                        {
                            var currentServiceChannelModel = parameter as LocalPictureLibraryServiceChannelModel;
                            DefaultViewModel =
                                new LocalPictureLibraryServiceChannelPageViewModel(this, pageState,
                                                                                   currentServiceChannelModel);
                        }
                    }
                }
            }

            if (CoreApplication.Properties.ContainsKey(ConstKeys.LOCAL_PICTURE_LIBRARY_VIEW_PAGE_KEY))
            {
                object parameter = CoreApplication.Properties[ConstKeys.LOCAL_PICTURE_LIBRARY_VIEW_PAGE_KEY];
                if (parameter is LocalPictureLibraryServiceChannelModel)
                {
                    var currentServiceChannelModel = parameter as LocalPictureLibraryServiceChannelModel;
                    DefaultViewModel =
                        new LocalPictureLibraryServiceChannelPageViewModel(this, pageState, currentServiceChannelModel);
                }
            }
        }
        ...
        private GridView GenerateItemGridView()
        {
            ...
            itemGridView.SetBinding(ItemsControl.ItemsSourceProperty,
                                    new Binding
                                        {
                                            Source =
                                                FindName("localPictureLibraryItemsViewSource") as CollectionViewSource
                                        });
