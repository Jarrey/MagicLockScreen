...
{
    public class InfoImageSearchServiceChannel : ServiceChannel, ISearchServiceChannel
    {
        private string settingsFileName = "InfoImageSearchService.setting";
        ...
        /// <summary>
        ///     ID is [Guid("475ADEF6-258F-427E-A0C2-5EB6C64DD5F0")]
        /// </summary>
        public InfoImageSearchServiceChannel(XmlElement configXml)
            : base(configXml)
        {
        }
        ...
        public async void InitializeServiceChannelModels()
        {
            ...
            try
            {
                ...
                if (await ApplicationData.Current.RoamingFolder.CheckFileExisted(settingsFileName))
                {
                    ...
                    {
                        AddModel(this, element.GetAttribute("Keyword"), ResourcesLoader.Loader["InfoSearchSubTitle"]);
                    }
                    xmlSetting = null;
                }
            }
        ...
        public void AddModel(ServiceChannel serviceChannel, string queryText, string subTitle)
        {
            var model =
                new ImageSearchServiceChannelModel(serviceChannel, queryText, subTitle)
                    {
                        SearchProviderType = SearchProvider.InfoDotCom
                    };
        ...
        protected override void InitializeServiceChannel(XmlElement configXml)
        {
            ...
            if (ID.CompareTo(new Guid("475ADEF6-258F-427E-A0C2-5EB6C64DD5F0")) != 0)
        ...
        public async void InitializeModelLogo(ImageSearchServiceChannelModel model)
        {
            try
            {
                ...
                IList<SearchImage> infoImages =
                    await infoImageSearchService.QueryDataAsync(model.Title,
                                                                (uint)random.Next(0, (int)(infoImageSearchService.MaxItemCount - LogoCount)),
                                                                LogoCount);
                if (infoImages != null && infoImages.Count > 0)
                {
                    model.Logo = new Collection<BindableImage>(infoImages as IList<BindableImage>);
                    ApplicationHelper.UpdateTileNotification(infoImages[0].ThumbnailImageUrl, model.Title,
                                                             infoImages[0].Title);
                }
            }
            catch (Exception ex)
