...
{
    public class BaiduImageCollection : ModelBase
    {
        ...
        public BaiduImageCollection(uint maxCount, IService[] services, string queryText)
            : base(services)
        {
            ...
            if (Images == null)
                Images = new RangeIncrementalLoadingClass<SearchImage>(maxCount,
                                                                       async (index, count) =>
                                                                       await LoadMoreBaiduImages(index, count));
        }
        ...
        private async Task<IList<SearchImage>> LoadMoreBaiduImages(uint index, uint count)
        {
            IsLoading = true;
            ...
            try
            {
                ...
                images = await baiduImageSearchService.QueryDataAsync(_queryText, index, count);
